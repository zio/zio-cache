...
inThisBuild(
  List(
    ...
    scmInfo := Some(
      ScmInfo(url("https://github.com/zio/zio-cache/"), "scm:git:git@github.com:zio/zio-cache.git")
    )
  )
)
...
lazy val zioCache = crossProject(JSPlatform, JVMPlatform)
  .in(file("zio-cache"))
  .settings(stdSettings("zioCache"))
